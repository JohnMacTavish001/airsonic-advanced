...
3. Step into the Dockerfile folder and build the image, make sure your docker is running
   ```pwsh
   docker build -t airsonic-advanced:original -f install/docker/Dockerfile install/docker/
   ```
   
4. run the docker image
   ```pwsh
   docker run -d --name airsonic-original  -p 4040:4040 -v .\install\docker\music:/var/music -v .\install\docker\airsonic:/var/airsonic  airsonic-advanced:original
   ```
